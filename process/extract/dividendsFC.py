...
def Extract( iCompany, iSoup ):
	...
	entries = iCompany.mFinances.mDividends
	...
	for entry in reversed( entries ):
		if entry.mType == company.company.cFinances.cDividend.eType.kDividend:
		...
		elif entry.mType == company.company.cFinances.cDividend.eType.kActionOnly:
			labels.append( 'AO' )
			data.append( '0' )
			colors.append( 'rgba( 255, 255, 255, 255 ) ' )
		elif entry.mType == company.company.cFinances.cDividend.eType.kSplit:
			labels.append( 'S' )
			data.append( '0' )
			colors.append( 'rgba( 255, 255, 255, 255 ) ' )
			last_price = entry.mPrice
